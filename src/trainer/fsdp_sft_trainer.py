...
from src.diffllm.gen_utils import q_sample, shuffle_think_blocks
...
def _get_hsms_cfg(diffusion_cfg):
    hsms_cfg = getattr(diffusion_cfg, "hsms", None)
    if hsms_cfg is not None:
        hsms_cfg = convert_to_regular_types(hsms_cfg)
    else:
        hsms_cfg = {}
    return hsms_cfg


def build_structure_special_tokens(diffusion_cfg):
    hsms_cfg = _get_hsms_cfg(diffusion_cfg)
    max_think_blocks = int(hsms_cfg.get("max_think_blocks", 6))

    tokens = []
    for i in range(1, max_think_blocks + 1):
        tokens.append(f"<think {i}>")
        tokens.append(f"</think {i}>")
    tokens.extend(["<think>", "</think>", "<summary>", "</summary>"])
...
def build_structure_cfg(tokenizer, diffusion_cfg):
    if not getattr(diffusion_cfg, "structured_masking", False):
        return None
    ...
    think_end_tags = []
    for i in range(1, max_think_blocks + 1):
        think_start_tags.append(
            tokenizer.encode(f"<think {i}>", add_special_tokens=False)
        )
        think_end_tags.append(
            tokenizer.encode(f"</think {i}>", add_special_tokens=False)
        )
    think_start_tags.append(tokenizer.encode("<think>", add_special_tokens=False))
    think_end_tags.append(tokenizer.encode("</think>", add_special_tokens=False))

    summary_start_tag = [tokenizer.encode("<summary>", add_special_tokens=False)]
    summary_end_tag = [tokenizer.encode("</summary>", add_special_tokens=False)]

    return {
        "think_start_tags": [t for t in think_start_tags if t],
        "think_end_tags": [t for t in think_end_tags if t],
        "summary_start_tag": [t for t in summary_start_tag if t],
        "summary_end_tag": [t for t in summary_end_tag if t],
        "rpim_r_max": float(hsms_cfg.get("rpim_r_max", 0.9)),
        "rpim_alpha": float(hsms_cfg.get("rpim_alpha", 0.8)),
        "rpim_kappa_think": float(hsms_cfg.get("rpim_kappa_think", 0.0)),
        "rpim_kappa_summary": float(hsms_cfg.get("rpim_kappa_summary", 1.0)),
        "shuffle_think": bool(hsms_cfg.get("shuffle_think", False)),
    }
...
class OptimizedCollateFunction:
    ...
    def _apply_perbatch_cutoff(
        ...
        if self.perbatch_cutoff_type == "random":
            ...
            cutoff_seq_len = self.np_random.choice(non_pad_lens)
            ...
            kept_response_len = self.np_random.choice(response_lens.cpu())
...
class FSDPSFTTrainer(object):
    ...
    def __init__(
        ...
        # Validation control flag (default: disabled)
        self.do_validation = getattr(self.config.trainer, "do_validation", False)
        ...
        structure_tokens = build_structure_special_tokens(self.config.diffusion)
        self.num_added_tokens = 0
        if structure_tokens:
            self.num_added_tokens = self.tokenizer.add_special_tokens(
                {"additional_special_tokens": structure_tokens}
            )
            if self.device_mesh.get_rank() == 0 and self.num_added_tokens > 0:
                print(f"Added {self.num_added_tokens} structure special tokens.")

        self.structure_cfg = build_structure_cfg(self.tokenizer, self.config.diffusion)

        model_name = str(self.config.model.partial_pretrain).lower()
        cfg_shift = getattr(self.config.diffusion, "shift_logits", None)
        if cfg_shift is None:
            self.shift_logits = "llada" not in model_name
        else:
            self.shift_logits = bool(cfg_shift)

        mask_token_id = getattr(self.config.model, "mask_token_id", None)
        if mask_token_id is None:
            mask_token_id = getattr(self.config.diffusion, "mask_token_id", None)
        if mask_token_id is None:
            mask_token_id = self.tokenizer.mask_token_id
        if mask_token_id is None and "llada" in model_name:
            mask_token_id = 126336
        if mask_token_id is None:
            raise ValueError(
                "mask_token_id is not set. Please set model.mask_token_id or "
                "diffusion.mask_token_id for diffusion training."
            )
        self.mask_token_id = int(mask_token_id)

        if self.tokenizer.mask_token_id is None:
            try:
                self.tokenizer.mask_token = self.tokenizer.decode([self.mask_token_id])
            except Exception:
                pass

        if self.tokenizer.pad_token_id == self.mask_token_id:
            raise ValueError("pad_token_id equals mask_token_id; please set a different pad token.")
    ...
    def _build_dataloader(self):
        ...
        if collate_fn_type == "streaming":
            ...
            val_collate_fn = (
                StreamingCollateFunction(config, self.tokenizer)
                if self.do_validation
                else None
            )
        else:
            ...
            val_collate_fn = (
                OptimizedCollateFunction(config, self.tokenizer)
                if self.do_validation
                else None
            )
    ...
    def _build_model_optimizer(self, checkpoint_path=None):
        ...
        with init_context():
            ...
            if getattr(self, "num_added_tokens", 0) > 0:
        ...
        with context:
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                if not use_sp:
                    ...
                    if self.structure_cfg and self.structure_cfg.get("shuffle_think", False):
                        input_ids = shuffle_think_blocks(
                            input_ids, loss_mask, self.structure_cfg
                        )
                    ...
                    masked_input_ids, t, loss_mask_nonflatten = q_sample(
                        ...
                        structure_cfg=self.structure_cfg,
                    )
